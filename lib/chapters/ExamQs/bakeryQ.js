...
const selector = {
    'loavesToFlour': ({loavesInM5, flourPerLoaf, names}) => {
        let ans = loavesInM5 * flourPerLoaf / 1000
        return {
            q: `${names[0]} uses ${flourPerLoaf}g of flour to make a loaf of bread. \n`+
                `How many Kgs of flour would ${names[0]} need to make ${loavesInM5} loaves of bread?`,
            a: ans,
            qFeedback: `${names[0]} needs ${ans}Kg of flour.`,
            hints: [
    ...
    'meanNoBags': ({ fiveMonthsBags, bagsM5, loavesInM5, flourPerBag, flourPerLoaf, names }) => {
...
const sectionSetup = (qName) => {
    ...
    return {
        qType: 'shortAnswer',
        ...selector[qName]({ fiveMonthsBags, bagsM5, loavesInM5, flourPerBag, flourPerLoaf, names })
    }
}

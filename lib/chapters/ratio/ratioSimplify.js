...
const selector = {
    noUnits1: (ratio, m) => {
        ...
        return {
            ...
            hint: `What divides ${ratio.length === 2 ? 'both' : 'each'} of the numbers in the ratio?`,
            giveAway: `You can divide the numbers in the ratio by ${m}`,
    ...
    noUnits2: (ratio, m, ms) => {
        ...
        return {
            ...
            hint: `You can divide each number in the ratio by ${ms[0]} to get ${qRatio.map(n => n / ms[0]).join(':')} but then you can simplify further.`,
            giveAway: `The HCF(${qRatio.join(',')}) is ${m}`,

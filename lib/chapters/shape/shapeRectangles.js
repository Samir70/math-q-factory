...
const selector = {
    areaFromLW: (len, wid, area) => {
        return {
            q: `What is the area of a rectangle with length ${len} and width ${wid}?`,
            a: area,
            hints: [
    ...
    wFromAreaL: (len, wid, area) => {
        return {
            q: `What is the width of a rectangle with length ${len} and area ${area}?`,
            a: wid,
            hints: [
            ...
            buildingBlocks: [
                'algebra01-solveLinear-oneStepMult'
            ],
    ...
    areaFromPerimSide: (len, wid, area, perim) => {
        return {
            q: `What is the area of a rectangle that has perimeter ${perim} and length ${len}?`,
            a: area,
            hints: [
            ...
            buildingBlocks: [
                'algebra01-solveLinear-oneStepMult', 'shape-rectangle-wFromPerimL'
            ],
    ...
    perimFromLW: (len, wid, area, perim) => {
        return {
            q: `What is the perimeter of a rectangle with length ${len} and width ${wid}?`,
            a: perim,
            hints: [
    ...
    wFromPerimL: (len, wid, area, perim) => {
        return {
            q: `What is the width of a rectangle with length ${len} and perimeter ${perim}?`,
            a: wid,
            hints: [
                `Two of the sides add up to ${len * 2}. \nWhat do the other two sides add up to?`,
            ...
            buildingBlocks: [
                'algebra01-solveLinear-oneStepAdd'
            ],
    ...
    perimFromAreaSide: (len, wid, area, perim) => {
        return {
            q: `What is the perimeter of a rectangle with area ${area} and length ${len}?`,
            a: perim,
            hints: [
                'Use the area to find the width of the rectangle'
            ],
            buildingBlocks: [
                'algebra01-solveLinear-oneStepAdd', 'shape-rectangle-wFromAreaL'
            ],
            qFeedback: `The width is ${wid} \nmaking the perimeter ${perim}`
        }
    }

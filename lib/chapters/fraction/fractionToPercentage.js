...
const selector = {
    quarters: ({ denomThatIsFactor }) => {
        ...
        return {
            q: `Convert ${wrapMJax(`${numer}/4`)} to a percentage`,
            a: ans,
            hints: [
                'Percent means per 100',
                `Can you convert ${wrapMJax(`${numer}/4`)} into something over 100?`,
                `What is ${wrapMJax(`${numer}/4`)} of 100?`
            ],
            buildingBlocks: [
                'fraction-ofAmount-manySlices-120',
                'fraction-simplify-composite-90'
            ],
            qFeedback: `${wrapMJax(`${numer}/4`)} = ${ans}`
        }
    },
    str8toOver100: () => {
        return {

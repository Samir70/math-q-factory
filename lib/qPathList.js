...
const shortAnswerPaths = {
    ...
    algebra01: {
        ...
        solveLinear: ['oneStepMult', 'oneStepAdd', 'twoStep1', 'twoStep2', 'brackets', 'unknownBothSides', 'fraction'],
    ...
    sequences: {
        linear: ['findNextTerm', 'giveTermToTermRule', 'useFormula1to5', 'useFormula100s', 'findFormula'],
...
const multiChoicePaths = {
    vocab: {
        ...
        circles: ['diameter', 'radius', 'tangent', 'chord'],
        // sequences: [''],
        // triangles: [''],
        // typesOfNumber: [''],
        // quadrilaterals: ['']
...
const pathsList = [shortAnswerPaths, multiChoicePaths, sortQPaths]

const topicsToTest = []
for (let qTypePaths of pathsList) {
    for (let chapter in qTypePaths) {
        totalQs[chapter] = totalQs[chapter] || 0
        for (let section in qTypePaths[chapter]) {
            totalQs[chapter] += qTypePaths[chapter][section].length
            for (let qName of qTypePaths[chapter][section]) {
...
console.log(totalQs)
module.exports = { topicsToTest, shortAnswerPaths, multiChoicePaths, totalQs }